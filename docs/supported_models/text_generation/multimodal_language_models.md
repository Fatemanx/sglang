...
| **BAGEL-7B-MoT**           | `ByteDance-Seed/BAGEL-7B-MoT`              | ByteDance-Seed's multimodal model. SGLang currently supports the understanding path for image-text inputs; BAGEL image generation and editing are not yet supported.                                            |  |
